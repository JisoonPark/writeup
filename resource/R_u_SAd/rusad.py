...
def keysize(string):
	try:
		...
		raise argparse.ArgumentTypeError('{string:r} is not a number')
	if value % 8:
		raise argparse.ArgumentTypeError('{value} must be a multiple of 8')
	if 256 <= value and value < 8192:
		return value
	raise argparse.ArgumentTypeError('{value} out of range')
...
def keyfile(private=False):
	def func(string):
		...
		if not isinstance(key, Key):
			raise argparse.ArgumentTypeError('{string:r} did not specify a valid key')
		if private and not key.ispriv():
			raise argparse.ArgumentTypeError('{string:r} did not specify a private key')
		return key
	return func
