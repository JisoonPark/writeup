...
f() 함수부터 살펴보면, 뭔가 값을 만드는 함수인 것 같은데, x()라는 함수가 호출한다. x() 함수는 순서대로 f(), l(), a(), g(), s() 함수를 호출한다.(!!) x() 함수는 d() 함수에서 호출하는데, d() 함수는 따로 호출해주는 함수가 없다.
...
그러고 나서 i VoMGer iW Pou GiG 부분을 봤는데, 이 부분이 I wonder if you did인것 같은 감이 강하게 외서, V:w, M:n, G:d, W:f, P:y를 추가하였다.
